<?php
/**
 * Gestion des règles de ciblage pour Elementor Supertool.
 *
...
class MJET_Target_Rules {
	...
	public function get_location_options() {
		$options = array(
			'basic'         => array(
				'label' => __( 'Basique', 'elementor-supertool' ),
				'value' => array(
					'basic-global'    => __( 'Site entier', 'elementor-supertool' ),
					'basic-singulars' => __( 'Toutes les pages singulières', 'elementor-supertool' ),
					'basic-archives'  => __( 'Toutes les archives', 'elementor-supertool' ),
				),
			),
			'special-pages' => array(
				'label' => __( 'Pages spéciales', 'elementor-supertool' ),
				'value' => array(
					'special-404'    => __( 'Page 404', 'elementor-supertool' ),
					'special-search' => __( 'Page de recherche', 'elementor-supertool' ),
					'special-blog'   => __( 'Page du blog', 'elementor-supertool' ),
					'special-front'  => __( 'Page d\'accueil', 'elementor-supertool' ),
				),
			),
			'post'          => array(
				'label' => __( 'Articles', 'elementor-supertool' ),
				'value' => array(
					'post|all'     => __( 'Tous les articles', 'elementor-supertool' ),
					'post|archive' => __( 'Archive des articles', 'elementor-supertool' ),
				),
			),
			'page'          => array(
				'label' => __( 'Pages', 'elementor-supertool' ),
				'value' => array(
					'page|all' => __( 'Toutes les pages', 'elementor-supertool' ),
				),
			),
		);
		...
		foreach ( $custom_post_types as $post_type ) {
			$options[ $post_type->name ] = array(
				...
				'value' => array(
					$post_type->name . '|all' => sprintf(
						...
						__( 'Tous les %s', 'elementor-supertool' ),
						$post_type->label
					),
				),
			);
			...
			if ( $post_type->has_archive ) {
				$options[ $post_type->name ]['value'][ $post_type->name . '|archive' ] = sprintf(
					/* translators: %s: post type name */
					__( 'Archive %s', 'elementor-supertool' ),
					$post_type->label
				);
			}
		}
	...
	public function get_user_role_options() {
		...
		$options = array(
			'all'        => __( 'Tous', 'elementor-supertool' ),
			'logged-in'  => __( 'Connecté', 'elementor-supertool' ),
			'logged-out' => __( 'Déconnecté', 'elementor-supertool' ),
		);
